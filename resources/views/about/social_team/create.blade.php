    ...
    <div class="col-md-12 col-sm-12" style="margin-bottom:30px; padding-left:40px; padding-top:15px; padding-right:20px; ">
        <header>
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                {{ __('Create Your Home Social Teams') }}
            </h2>

            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400" style="margin-bottom: 15px">
                {{ __('Add Social For your Teams informations.') }}
            </p>
        </header>
        <form method="post" action="{{ route('socialteams.store') }}" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="teams" value="{{ $teams->id }}">
            ...
            <div class="flex justify-end col-mt-8">
                ...
                <a href="{{ route('socialteams.crud',['id' => $teams]) }}" class="bg-red-500 text-white ml-4 px-4 py-2 rounded">Kembali</a>
            </div>
        </form>
    </div>
@endsection
