...
<div class="col-md-12 col-sm-12 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6" style="margin-bottom:30px; padding-left:40px; padding-top:15px; padding-right:20px;">
        ...
        <form method="post" action="{{ route('materi.update',$modul->id) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            {{-- <input type="hidden" value="{{ $materi->id }}" name="id"> --}}
            <div class="form-group">
                <label>Judul</label>
                <input type="text" class="form-control" value="{{ old('judul', $modul->judul) }}" name="judul"
                    placeholder="Judul artikel">
            </div>
            <div class="form-group">
                <label>Deskripsi</label>
                <textarea class="form-control summernote" name="deskripsi" rows="10">{{ $modul->deskripsi }}
            </textarea>
            </div>
            <div class="form-group">
                <label>Image</label>
                <img id="previewImage" src="{{ asset('storage/' . $modul->image_path) }}" alt="gambar"
                    style="max-width: 200px; max-height: 200px;">
                <input type="file" class="form-control m-2" value="{{ old('image', $modul->image_path) }}"
            ...
            <div class="form-group">
                ...
                <input type="datetime-local" class="form-control" name="scheduled_at" id="scheduled_at" value="{{$modul->scheduled_at  ?: \Carbon\Carbon::now()}}">
            ...
            <div class="form-group">
                ...
                <select name="status" id="status" class="form-control">
                    <option value="0" {{ old('status', $modul->draft ?? 0) == 0 ? 'selected' : '' }}>Publikasi</option>
                    <option value="1" {{ old('status', $modul->draft ?? 0) == 1 ? 'selected' : '' }}>Draft</option>
                </select>
            </div>
